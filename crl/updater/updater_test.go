...
func TestGetWindowForShard(t *testing.T) {
	...
	start, end := tcu.getWindowForShard(zeroTime.Add(time.Minute), 0)
	...
	start, end = tcu.getWindowForShard(zeroTime.Add(time.Minute), 93)
	...
	start, end = tcu.getWindowForShard(zeroTime.Add(100*24*time.Hour+time.Minute), 0)
	...
	start, end = tcu.getWindowForShard(zeroTime.Add(100*24*time.Hour+time.Minute), 93)
	test.AssertEquals(t, start, zeroTime.Add(93*24*time.Hour))
	test.AssertEquals(t, end, zeroTime.Add(94*24*time.Hour))
	start, end = tcu.getWindowForShard(zeroTime.Add(100*24*time.Hour+time.Minute), 92)
